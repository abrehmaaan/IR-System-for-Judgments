...
with open("tfidf.txt") as f:
    for line in f:
        ...
        tfidf[term_id] = {}
        for pair in parts[1:]:
            doc_id, tfidf_val = pair.split(":")
            tfidf[term_id][doc_id] = float(tfidf_val)
...
for term_id, doc_dict in tfidf.items():
    for doc_id, tfidf_val in doc_dict.items():
        ...
        doc_norm_dict[doc_id] += tfidf_val ** 2
...
for term_id, doc_dict in tfidf.items():
    ...
    for doc_id, tfidf_val in doc_dict.items():
        normalized_tfidf_dict[term_id][doc_id] = tfidf_val / doc_norm_dict[doc_id]

# Write normalized tfidfweighting dictionary to file
with open('normaltfidf.txt', 'w') as f:
    for index, term_dict in normalized_tfidf_dict.items():
        line = f"{index}"
...
query_ids = list(set([query_id for term_index in normalized_tfidf_queries_dict for query_id in normalized_tfidf_queries_dict[term_index]]))
...
with open("similaritiestfidf.txt", "w") as f:
...
with open("top10tfidf.txt", "w") as f:
